...
class CommandLineInterface:
    ...
    def init(self):
        ...
        dict_args =vars(args)
        try:
            DirectoryBuilder(dark=dict_args["dark"], directory = dict_args["dir"], overwrite = dict_args["overwrite"]).build()
        except (FileExistsError, OSError) as e:
            logging.info(e)
            logging.info('Run the command with the `--overwrite` flag to ignore these errors and overwrite existing files.')
